  ...
  def question_creation(self):
    ...
    for i in range(len(self.periods)):
      ...
      if(len(self.periods[i])>0):
        ...
        if n_quest==1:
          prompt="Can you produce one question about this text considering the context without write anything more and with out any introduction? Text:"+self.periods[i]+" Context:"+' '.joint(self.periods)
          ...
          prompt="Can you produce "+str(n_quest)+" differents questions about this text considering the context without write anything more separeted by the sign '£' with out any introduction? Text:"+self.periods[i]+" Context:"+' '.joint(self.periods)
          output_model=self.model_llama.print_prompt(prompt)
          print(output_model)
